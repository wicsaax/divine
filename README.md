...
已完成:
- [x] **八字深入** — 十神 (天干 + 地支藏干) + 大运 + 起运推算. lunar 库支持
- [x] **塔罗牌面 + 翻牌动画** — 程序化绘制的牌面 + 翻转入场动画 (公版图片资源后续接入是一行 widget 切换)
- [x] **Android 前台服务保活网络** — LLM 流式期间常驻通知, 切到别的 app 也不断流

待做 (按复杂度):
- [ ] **紫微斗数** — 14 主星安星法 + 12 宫 + 三方四正 + 四化. 完整版至少 1-2 天工程, 需要单独排期, 不在 v0.3 范围
- [ ] **占星本命盘真算** — Swiss Ephemeris (sweph) Dart 绑定, 行星精确黄经 + Placidus 宫位 + 相位. 涉及 FFI 和星历数据文件, 跨平台调试需要 1-2 天
- [ ] **真实韦特塔罗牌面图片** — 替换当前程序化牌面. 需要找/制作 78 张公版图. 2-3 小时
